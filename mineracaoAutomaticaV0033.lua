...
function navegarParaVolta(contador)
    -- Proteção contra loops infinitos
    contador = contador or 0
    if contador > 100 then
        print("ERRO: Muitas tentativas de movimento na volta, parando...")
        return false
    end
    
    print("Navegando de volta - Tentativa: ", contador)
    
    -- Verifica se o bloco abaixo é válido
    if blocoValidoAbaixo() then
        if turtle.forward() then
            print("Andando de volta sobre bloco válido...")
            return navegarParaVolta(contador + 1)
        else
            print("Não conseguiu andar para frente na volta")
            return false
        end
    else
        -- Se não for bloco válido, volta para o bloco anterior
        print("Bloco válido não encontrado na volta, voltando...")
        turtle.back()

        -- Na volta, sempre tenta virar à esquerda primeiro
        turtle.turnLeft()
        
        if blocoValidoAbaixo() then
            if turtle.forward() then
                print("Encontrou bloco válido à esquerda, continuando volta...")
                return navegarParaVolta(contador + 1)
            end
        else
            -- Se não encontrar à esquerda, tenta à direita
            turtle.turnRight()  -- Desfaz turnLeft
            turtle.turnRight()  -- Vira à direita
            
            if blocoValidoAbaixo() then
                if turtle.forward() then
                    print("Encontrou bloco válido à direita, continuando volta...")
                    return navegarParaVolta(contador + 1)
                end
            else
                -- Se não encontrar em nenhum lado, volta para a posição original
                turtle.turnLeft()  -- Desfaz turnRight
                print("Nenhum bloco válido encontrado em nenhuma direção na volta")
                return false
            end
        end
    end
...
function voltarParaMineracao()
    ...
    print("Iniciando volta para mineração...")
    
    -- Usa a navegação específica para volta
    local voltouComSucesso = navegarParaVolta()
    
    if voltouComSucesso then
        -- Ajusta posição para minerar novamente
        turtle.turnRight()
        turtle.turnRight()
        print("Voltou para a posição de mineração!")
    else
        print("ERRO: Não conseguiu voltar para a posição de mineração")
        -- Tenta uma recuperação simples
        turtle.turnRight()
        turtle.turnRight()
        print("Tentando recuperação...")
    end
end
...
function main()
    ...
    while true do
        ...
        if chegouAoBau then
            ...
            voltarParaMineracao()
            pos_chest = false  -- Reseta para a próxima ida ao baú
        else
